...
data_path = '/Users/luozhongyi/Desktop/2015 Fall course/CS 6476/proj5/data';  %change if you want to work with a network copy
...
[tp_rate, fp_rate, tn_rate, fn_rate] =  report_accuracy_2( confidences, label_vector );
...
hn_thresh=0.9; %before mirror is 0.8
...
for k=1:1:length(hn_index)
    hn_image = im2single(rgb2gray(imread(strcat(non_face_scn_path, '/', hn_image_ids{k}))));
    hn_xran=size(hn_image,2);
    hn_yran=size(hn_image,1);
...
%%substract most hn_makeup
%%retrain:
fprintf('Retrain classifier performance on train data:\n')
lambda = 0.0001; %%former 0.0001
...
confidences = [features_pos; features_neg;hn_makeup]*wt + bt;
label_vector = [ones(size(features_pos,1),1); -1*ones(size(features_neg,1),1);-1*ones(size(hn_makeup,1),1)];
[tp_rate, fp_rate, tn_rate, fn_rate] =  report_accuracy_2( confidences, label_vector );
...
[bboxes, confidences, image_ids] = run_detector(test_scn_path, wt, bt, feature_params);
